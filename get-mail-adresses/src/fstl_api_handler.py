...
class fstl_api:
    ...
    def get_users(self, fields=[""], group=[""], role=[""], status=["active"]) -> list:
        """
        ...
        :return: if success: json object with the users, else: False
        """
        users = []
        page = 0
        size = 40  # Setzen Sie die Größe nach den Möglichkeiten Ihrer API
        params = {
            "addressResult": "none",  # we don't need addresses here
            "roles": role,
            "statuses": status,
            "groups": group,
            "page": page,
        ...
        while True:
            ...
            if evaluate_response(response):
                ...
                users.extend(data)  # Nehmen wir an, dass die Antwort direkt die Nutzerliste ist
                if len(data) < size:
                    # Wenn weniger Daten zurückgegeben werden als erwartet, sind wir am Ende der Liste
                    break
                page += 1
                params["page"] = page
            else:
                return evaluate_response(response)  # Oder behandeln Sie Fehler entsprechend
        return users
